...
* Multi-Benchmarks dataset: This dataset is a large patch dataset that involves patches for three bug benchmarks: Defects4J, Bugs.jar, and Bears. The construction process of the Multi-Benchmarks dataset is as follows:
...
```
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 python /ablation_study/Graph_LoRA_ablation/Graph_LoRA-Attention/train.py   --model_id /path/of/LLM   --train_json /path/of/train/data   --val_json   /path/of/test/data   --max_len 1536 --micro_batch_size 2 --global_batch_size 2   --epochs 1 --lr 2e-4 --warmup_ratio 0.06   --rank 64 --target all   --output_dir result_nocompile_nofa

```
(2)Graph-LoRA-Weak: We remove GNN and directly input the linearized APSG content into the LLMs. 
```
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 python /ablation_study/Graph_LoRA_ablation/Graph_LoRA-Week/train.py   --model_id /path/of/LLM   --train_json /path/of/train/data   --val_json   /path/of/test/data   --max_len 1536 --micro_batch_size 2 --global_batch_size 2   --epochs 1 --lr 2e-4 --warmup_ratio 0.06   --rank 64 --target all   --output_dir result_nocompile_nofa
...
```
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 python /ablation_study/Graph_LoRA_ablation/APSG-Atteibute/train.py   --model_id /path/of/LLM   --train_json /path/of/train/data   --val_json   /path/of/test/data   --max_len 1536 --micro_batch_size 2 --global_batch_size 2   --epochs 1 --lr 2e-4 --warmup_ratio 0.06   --rank 64 --target all   --output_dir result_nocompile_nofa

```
(4)APSG-Graph: We remove the whole APSG and only input the code patches into the LLMs.
```
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 python /ablation_study/Graph_LoRA_ablation/APSG-Graph/train.py   --model_id /path/of/LLM   --train_json /path/of/train/data   --val_json   /path/of/test/data   --max_len 1536 --micro_batch_size 2 --global_batch_size 2   --epochs 1 --lr 2e-4 --warmup_ratio 0.06   --rank 64 --target all   --output_dir result_nocompile_nofa
```
...
```
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 python /ablation_study/Graph_LoRA_ablation/Original-LLM.py --input input.jsonl --output output.jsonl --model_name LLM_name --model_path /path/of/LLM
```
